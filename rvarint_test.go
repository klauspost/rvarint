...
func TestBufferTooBigWithOverflow(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:        reverseBytes([]byte{0xd7, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x01}),
			wantValue: math.MaxUint64 - 40,
			wantN:     10,
		},
		{
			name:      "invalid: with more than MaxVarintLen64 bytes",
			in:        reverseBytes([]byte{0xd7, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x01}),
			wantN:     -11,
			wantValue: 0,
		},
		{
			name:      "invalid: 10th byte",
			in:        reverseBytes([]byte{0xd7, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f}),
			wantN:     -10,
			wantValue: 0,
		},
	}
...
func testOverflow(t *testing.T, buf []byte, x0 uint64, n0 int) {
	x, n := Uvarint(buf)
	if x != 0 || n != n0 {
...
func TestOverflow(t *testing.T) {
	testOverflow(t, reverseBytes([]byte{0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x2}), 0, -10)
	testOverflow(t, reverseBytes([]byte{0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x1, 0, 0}), 0, -11)
	testOverflow(t, reverseBytes([]byte{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}), 1<<64-1, -11) // 11 bytes, should overflow
}

func TestNonCanonicalZero(t *testing.T) {
	buf := reverseBytes([]byte{0x80, 0x80, 0x80, 0})
	x, n := Uvarint(buf)
	if x != 0 || n != 4 {
